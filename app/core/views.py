...
@extend_schema(
    description="Get businesses by license type and range of license creation/expiration dates. Date format should be like: yyyy-mm-dd",
    parameters=[
        OpenApiParameter(
            name="license_type",
            required=True,
...
def business_license_type(request):
    ...
    if (
        ...
        filter_kwargs["license_type"] = license_type
        instance = Business.objects.filter(**filter_kwargs)
        count_businesses = Business.objects.filter(**filter_kwargs).count()
        serializer = BusinessSerializer(instance, many=True)
        return Response(
